...
public class LoginInterceptor extends HandlerInterceptorAdapter{
	...
	//요청이 처리된 후에!
	//우리 예제에서는 loginPost2요청의 loginPost()메소드안의 내용을 다 실행하고
	//loginPost2.jsp페이지 보여주기 전에 아래 postHandle처리를 수행한다.
	//정리 :
	//어딘가에서 loginPost2요청하면 위의 preHandle()메소드 실행하고, loginController2의 loginPost()메소드 실행, 아래 postHandler()메소드 실행한 후 loginPost2.jsp페이지 보여줌
	@Override
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		if(userVO!=null) {
			...
			}else{ //사용자가 로그인아이디 기억을 체크하지 않았을 경우
				//기존의 쿠키값 지우기
		...
		//AuthInterceptor 생성 후 추가할 부분
		Object dest = session.getAttribute("dest"); //destination
